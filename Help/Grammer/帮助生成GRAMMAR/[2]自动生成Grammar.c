...
int main(void)
{
	freopen("My_C_Grammar.c", "r", stdin);
	freopen("result.c", "w", stdout);		  
	
	int i = 0, count = 0;
	char str[81];
	char left[81], right[81];
	for(i = 1; scanf("%s", str) > 0; i++)			// 一行读取一个语法符号 
	{
		if(strcmp(str, "") == 0)
		{
			// printf("\n\n");
		...
		if(str[strlen(str)-1] == ':' && strlen(str) > 1)	// 如果遇见的是产生式的左部 
		{
		//	scanf("%[null]", str);
	//		count = 0; 
			str[strlen(str)-1] = '\0';
		...
		else if(strcmp(str, "end_null") == 0)			// 结束标志 
		{
			if(count != 0)
			{
				printf("	%d\n\n", count);
		...
		else if(str[strlen(str) - 1] >= '0' && str[strlen(str) -1]  <= '9')
		{
			printf("\n%s  %s	-=>	", str, left);	// str是当前产生式的编号 
		//	count ++;
		} 
		else if(Display(str) == 0)		// 是语法变量 
		{
			printf("%s IS_TOKENS_DATA,    ", str);
			count++;
		}                         
		else							// 表明当前位置是文法符号
		{
			printf("%s, IS_SYNANA_DATA,    ", str);
			count++;
		}
	}
...
int Display(char *received)
{
	...
	{	
		strcpy(received,"RSHIFT_ASSIGN_OPERATE,");	
	}
	else if(strcmp(received, "<<=") == 0)	
	{	
		strcpy(received,"LSHIFT_ASSIGN_OPERATE,");	
	}
	else if(strcmp(received, "||") == 0)	
	{	
