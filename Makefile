...
#  PROP
PROP_DLL_ELF = $(BIN_ROOT_DIR)prop-dll.exe
...
target : $(PROP_DLL_ELF) $(PROP_LIB_ELF) 

all	:	target


# Use the Dynamic Link Library libCTools.so to complie the EIF
$(PROP_DLL_ELF): $(OBJ_GLOBAL_FILE) $(DLL)
	make res
	gcc $(OBJ_GLOBAL_FILE) $(PROP_RES) -o $@  -L$(LIB_DIR) -lCTools

$(PROP_LIB_ELF): $(OBJ_FILES)
	gcc $^ $(PROP_RES) -o $@
